...
# Mapping for symptoms and diseases
symptoms_dict = {
...
def predict():
    try:
        data = request.json  # Expecting a list of symptom inputs
        ...
        response = {
            ...
            'medications': med,
            'diets': die,
            'workout': wrkout.values.tolist() if not wrkout.empty else []
        }

        # Convert all non-serializable data to lists for JSON response
        response['precautions'] = [list(p) for p in response['precautions']]
        response['workout'] = response['workout'].tolist() if isinstance(response['workout'], np.ndarray) else response['workout']
